...
const ImageViewer = ({ product, onClose, onProductClick }: ImageViewerProps) => {
  ...
  // For jeans/no-brand products, hide the "more from brand" section entirely
  const moreBrandProducts = (product.category === 'jeans' || !product.brand)
    ? []
        ...
        {finalAlsoLike.length > 0 && (
          <div className="px-3 md:px-8 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-3">
              {finalAlsoLike.filter((p): p is Product => !!p).map((relatedProduct) => (
                <button
                  key={relatedProduct.id}
                  onClick={() => handleRelatedProductClick(relatedProduct)}
                  className="text-left"
                >
                  ...
                  <p className="brand-name text-center">
                    {relatedProduct.category === 'jeans' || !relatedProduct.brand ? 'ESPACIO JEANS' : getBrandDisplayName(relatedProduct.brand)}
                  </p>
                </button>
              ))}
            </div>
