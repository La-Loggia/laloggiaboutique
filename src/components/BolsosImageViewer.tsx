...
const BolsosImageViewer = ({ product, onClose, onProductClick }: BolsosImageViewerProps) => {
  ...
  const { data: brandBolsos = [] } = useBolsosByBrand(product.brand as BolsoBrand);
  const { data: otherBolsos = [] } = useBolsosExcludingBrand(product.brand as BolsoBrand, 6);
  ...
  const moreBrandProducts = brandBolsos
    .filter(p => p.id !== product.id)
    .slice(0, 10);

  // Filter out current product from other brands
  const alsoLikeProducts = otherBolsos
    .filter(p => p.id !== product.id)
    .slice(0, 6);
  ...
  return (
    <div ref={contentRef} className="fixed inset-0 z-[100] bg-background overflow-y-auto">
      ...
      {/* WhatsApp button */}
      <WhatsAppButton href={whatsappUrl} fixed label="Preguntar en La Loggia más información sobre este bolso" />
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 z-[210] p-4 bg-gradient-to-t from-white via-white/80 to-transparent pt-8">
            <a
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <span>Preguntar sobre este bolso</span>
...
export default BolsosImageViewer;
